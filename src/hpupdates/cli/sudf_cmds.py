...
def scan(
    ...
    console.print(
        f"[cyan]Scanning for updates (SysID={profile.sys_id}, OS={profile.os_code})...[/]"
    )
...
def bios_check(
    ...
    console.print(
        f"[cyan]Current BIOS: {profile.bios_version} (dated {profile.bios_release_date})[/]"
    )
...
def softpaq_download(
    ...
    destination: Annotated[
        Path, typer.Option("--destination", "-d", help="Download directory.")
    ] = Path("."),
    checksum: Annotated[
        str, typer.Option("--checksum", help="Expected MD5 checksum (optional).")
    ] = "",
) -> None:
    ...
    url = softpaq if softpaq.startswith("http") else _softpaq_url(sp)
...
def softpaq_install(
    ...
    apply: Annotated[
        bool, typer.Option("--apply", help="Actually install (default is dry-run).")
    ] = False,
...
def update_all(
    apply: Annotated[
        bool, typer.Option("--apply", help="Actually install (default: dry-run).")
    ] = False,
    country: Annotated[str, typer.Option("--country")] = "",
    language: Annotated[str, typer.Option("--language")] = "",
) -> None:
    ...
    if not apply:
        ...
        for u in needed:
            console.print(
                f"  • {u.get('Code', '')} — {u.get('Title', '')[:60]} v{u.get('Version', '')}"
            )
...
def download_all(
    destination: Annotated[Path, typer.Argument(help="Download directory.")] = Path(
        "./hp-downloads"
    ),
    country: Annotated[str, typer.Option("--country")] = "",
    language: Annotated[str, typer.Option("--language")] = "",
    ...
    for i, u in enumerate(needed, 1):
        ...
        try:
            ...
            if result.status in (DownloadStatus.Downloaded, DownloadStatus.AlreadyDownloaded):
                ...
                results.append(
                    {"code": code, "title": title, "file": result.local_path, "status": "ok"}
                )
                success += 1
            else:
                console.print(f"    [red]FAIL: {result.status.name}[/]")
                results.append(
                    {"code": code, "title": title, "file": "", "status": result.status.name}
                )
                failed += 1
        except Exception as exc:
            console.print(f"    [red]ERROR: {exc}[/]")
